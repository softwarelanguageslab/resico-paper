...
function read_failures(dataset_name::String, folder_name::String, mapping::Dict{Int64, String}, needs_mapping::Bool)
    if needs_mapping
        lines_data = readlines("/Users/kmilo/Dev/PhD/RESICO_new/rq2/results/$folder_name/knn/1/$dataset_name.txt")
    else
...
function main()
    ...
    for line in unique_mapping
        line_divided = split(line, ",")
        key_converted = parse(Int64, line_divided[1]) - 1 # This subtraction is needed
    ...
    for external_dataset in external_datasets
        ...
        # For RESICO the mapping is needed
        # failures = read_failures(external_dataset, FOLDER_NAME, fqns_dict, true)

        # For COSTER the mapping is not needed
        failures = read_failures(external_dataset, FOLDER_NAME, fqns_dict, false)
